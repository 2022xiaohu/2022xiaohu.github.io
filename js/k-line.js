...
var get_m_data = function(m_data, type) {
    ...
    $.each(m_data.trend, function(i, v) {
        priceArr.push(v[1]);
        avgPrice.push(v[2]);
        vol.push(v[3]);
    })
 ...
 */
function initMOption(m_data, name, type) {
    ...
    return {
        ...
        tooltip: { //弹框指示器
            ...
            formatter: function(params, ticket, callback) {
                ...
                var color;
                if (m_datas.priceArr[i] > m_data.preclose_px) {
                ...
                html += '<div>涨幅 <span  ' + color + ' >' + ratioCalculate(m_datas.priceArr[i], m_data.preclose_px) + '%</span></div>';
        ...
        yAxis: [ //y轴
            {
                ...
                axisLabel: { //label文字设置 
                    ...
                    // fontWeight: 'bold',
                    color: function(val) {
                        if (val == m_data.preclose_px) {
                            return '#aaa'
                        }
                        return val > m_data.preclose_px ? upColor : downColor;
                    }
                },
                axisLine: { //坐标轴轴线相关设置
                    show: true,
                ...
                axisLabel: { //label文字设置
                    ...
                    formatter: function(val) {
                        var resul = ratioCalculate(val, m_data.preclose_px);
                ...
                axisPointer: {
                    ...
                    label: {
                        formatter: function(params) { //计算右边Y轴对应的当前价的涨幅比例
                            return ratioCalculate(params.value, m_data.preclose_px) + '%';
                        }
                    }
                },
                axisLine: { //坐标轴轴线相关设置
...
function update_k_option(cdata) {
    var data = splitData(cdata);
    // var macd = calcMACD(12, 26, 9, data.datas, 1);
    return {
        series: [{
